...
# Check args
if [ $# -lt 2 ]; then
    echo "SilentPiper - Lite iDempiere Modeler with SQLite Staging"
    echo "========================================================="
    echo ""
    echo "Usage: $0 <sqlite.db> <command> [args...]"
    echo ""
    echo "AD MODEL (sync iDempiere → SQLite):"
    echo "  sync-ad                  - Pull AD model from PG to SQLite"
    echo "  info                     - Show AD model info in SQLite"
    echo ""
    echo "SQL DATA (SQL → SQLite → PostgreSQL):"
    echo "  sql2pack <data.sql>      - Parse SQL, stage in SQLite"
    echo "  packs [name]             - Show staged data packs"
    echo "  apply-data <pack>        - Insert data directly to iDempiere PG"
    echo "  applied <pack>           - Show applied records (tracked)"
    echo "  clean-data <pack>        - Delete applied records (cascading)"
    echo "  export-2pack <name> [dir]- Generate 2Pack.zip for distribution"
    echo ""
    echo "MODEL STAGING (Excel → SQLite → iDempiere):"
    ...
    echo ""
    echo "Examples:"
    echo "  $0 ninja.db sync-ad                    # Sync AD model"
    echo "  $0 ninja.db sql2pack GardenWorld.sql   # Stage SQL data"
    echo "  $0 ninja.db packs                      # List data packs"
    echo "  $0 ninja.db export-2pack GardenWorld   # Create 2Pack.zip"
